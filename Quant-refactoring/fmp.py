...
class FMP:
    ...
    def get_fmp_data(self, main_url, extra_url, need_symbol, is_v4, file_postfix=""):
        ...
        if len(fmp_info_list) == 1:
            ...
            # Limit processes to avoid API rate limits (was: cpu_count())
            # Recommended: 4-8 processes for external API calls
            max_processes = min(8, multiprocessing.cpu_count())
            with Pool(processes=max_processes, initializer=install_mp_handler()) as pool:
