...
최근 버그 수정 및 기능 개선으로 다음 문제들이 해결되었습니다:

**1. 무한대 값 처리 (PR #56, #60, #62)**
- ✅ **문제**: VIEW 데이터에 무한대 값 포함으로 2024-2025년 9개 분기 사용 불가
- ✅ **해결**:
  - fs_table과 metrics_table 로드 시 무한대 값을 자동으로 NaN으로 변환
  - 스케일링 전후 무한대 값 자동 제거
  - 상세 추적 CSV 출력 기능 추가 (`EXPORT_NAN_REMOVAL_DETAILS`)
- ✅ **효과**: 2024-2025년 데이터를 ML 학습에 사용 가능하게 됨

**2. Q1 분기 파일 생성 (PR #62)**
- ✅ **문제**: 6개 Q1 분기 파일 누락 (2015, 2016, 2019, 2020, 2021, 2022)
- ✅ **해결**:
  - 리밸런싱 날짜 없을 때 분기 말일을 기본값으로 사용 (3/31, 6/30, 9/30, 12/31)
  - 예측 전용 Parquet 파일 생성 (학습용 라벨 없음)
- ✅ **효과**: 6개 분기 파일 추가 확보 (1.5년 치 예측 데이터)

**3. NaN 데이터 추적 (PR #58, #59)**
- ✅ **문제**: NaN 제거 과정 불투명, 데이터 손실 파악 어려움
- ✅ **해결**:
  - 컬럼별 NaN 비율 상세 분석 로깅
  - NaN 제거 상세 정보를 CSV로 출력하는 진단 기능 추가
  - `EXPORT_NAN_REMOVAL_DETAILS: Y` 옵션으로 활성화 가능
- ✅ **효과**: 데이터 품질 진단 및 개선 방향 파악 용이

**4. VIEW 재구성 기능 (PR #47)**
- ✅ **추가**: FMP API 없이 기존 rawpq 데이터로 VIEW 재생성 기능
- ✅ **설정**: `MAKE_VIEW: Y` 옵션
- ✅ **효과**: API 할당량 절약, 빠른 데이터 재가공

---

#### 🔄 여전히 남은 문제

다음 데이터 품질 문제는 여전히 개선이 필요합니다:

- **결측치**: FMP API의 데이터 결측치 (특히 소형주, 신규상장 종목)
- **정확성 오류**: 일부 재무지표의 부정확함 (API 제공사 오류)
- **상장폐지 종목**: 상장폐지 후 데이터 불완전
